...
private:
  ...
  void heapify(std::vector<T> &list, int n, int i);
...
template <class T> std::vector<T> DLinkedList<T>::heapSort() {
  DLLNode<T> *current = head;
...
template <class T>
void DLinkedList<T>::heapify(std::vector<T> &list, int n, int i) {
  int largest = i;
